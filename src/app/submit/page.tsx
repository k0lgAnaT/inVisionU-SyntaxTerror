...
export default function SubmitPage() {
  ...
  const radarData = result ? [
    { dim: t('metric_leadership'), score: result.scores.leadership.score },
    { dim: t('metric_motivation'), score: result.scores.motivation.score },
    { dim: t('metric_growth'), score: result.scores.growth.score },
    { dim: t('metric_communication'), score: result.scores.communication.score },
    { dim: t('metric_velocity'), score: result.scores.growthVelocity.score },
  ] : [];
      ...
      <main className="min-h-screen pt-20 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="mb-8 animate-fade-in-up">
            ...
            <p className="text-slate-600 text-sm">{t('sub_test_desc')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 animate-fade-in-up delay-100">
              ...
              <div className="glass-card-static p-5">
                <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider mb-4">{t('cand_profile_details')}</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-600 mb-1 block">{t('auth_full_name')}</label>
                    <input className="input-field" placeholder="..." value={form.name}
                      onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-slate-600 mb-1 block">{t('auth_city')}</label>
                    <input className="input-field" placeholder="..." value={form.city}
              ...
              <div className="glass-card-static p-5">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-semibold text-slate-800">{t('sub_essay')}</label>
              ...
              <div className="glass-card-static p-5">
                <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wider mb-4">{t('sub_experience')} & {t('sub_achievements')}</h3>
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-slate-600 mb-1 block">{t('sub_experience')}</label>
                    <textarea className="input-field" rows={3} placeholder="..."
                      value={form.experience} onChange={e => setForm(f => ({ ...f, experience: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-slate-600 mb-1 block">{t('sub_achievements')}</label>
                    <textarea className="input-field" rows={2} placeholder="..."
            ...
            <div className="animate-fade-in-up delay-200">
              ...
              {!scoring && !result && (
                <div className="glass-card p-10 text-center">
                  ...
                  <p className="text-slate-600 text-sm">{t('sub_ready')}</p>
              ...
              {!scoring && result && (
                <div className="space-y-4 animate-fade-in">
                  ...
                  {/* Dimension Scores */}
                  <div className="glass-card p-5 bg-white">
                    <h4 className="text-sm font-semibold text-slate-800 mb-4">{t('cand_score_breakdown')}</h4>
                    <div className="space-y-3">
                  ...
                  {result.flags.length > 0 && (
                    <div className="glass-card p-5 bg-white">
                      <h4 className="text-sm font-semibold text-slate-800 mb-3">{t('cand_flags')}</h4>
                  ...
                  {result.smartQuestions.length > 0 && (
                    <div className="glass-card p-5 bg-white">
                      <h4 className="text-sm font-semibold text-slate-800 mb-3">{t('sub_questions')}</h4>
